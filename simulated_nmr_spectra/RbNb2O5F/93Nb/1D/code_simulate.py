    ...
    with open(filename, 'r') as file:
        for line in file:
            ...
            if line.strip() and not line.startswith('#'):
                ...
                delta_csa = float(data[3]) * -1  # 4th column
...
def simulate_and_save_chunks(input_filename, chunk_size=4):
    ...
    for i in range(num_chunks):
        ...
        method = BlochDecayCTSpectrum(
            ...
            rotor_frequency=100e3,  # in Hz (10 kHz as specified)
            spectral_dimensions=[
                SpectralDimension(
                    count=1000,
                    spectral_width=5e6,  # in Hz
                    label=r"$^{93}$Nb resonances",
                )
            ],
        )
        ...
        processor = sp.SignalProcessor(
            operations=[
                ...
                sp.apodization.Gaussian(FWHM="10000 Hz"),
                sp.FFT(),
            ]
        )
...
def plot_combined_and_experimental_spectrum(chunk_files, experimental_file, operating_frequency_mhz):
    ...
    experimental_data = pd.read_csv(
        ...
        names=['Frequency (Hz)', 'Intensity', 'Model', 'Line']  # Assign column names
    )

    # Convert Frequency from Hz to ppm
    experimental_data['Delta_ISO (ppm)'] = experimental_data['Frequency (Hz)'] / operating_frequency_mhz
    experimental_data['Intensity x 200'] = experimental_data['Intensity']
    ...
    # Plot settings
    plt.title("Simulated MAS NMR Spectrum of $^{93}Nb$")
...
# Main execution
input_filename = 'models_2-3-4-8-9_Nb.gipaw' #'data_test.gipaw'
