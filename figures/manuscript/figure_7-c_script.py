...
def P(C_Q, eta_Q):
    return ((C_Q**4 * eta_Q) / (np.sqrt(2 * np.pi) * (0.99541**5))) * ( 1 - 
        (eta_Q**2 / 9)) * np.exp(- (C_Q**2 / (2*(0.99541**2))) * (1 + (eta_Q**2 / 3))) # 995.42 (RbTa) and 825.98 (RbNb) 87Rb and 25783.45 (RbNb) 93Nb
...
canvas.view_init(90, -125) # To view the 3D graph from a specific angle
...
canvas.text(x=2, y=0.5, z=0.42, s=r'$^{87}$Rb $-$ RbTa$_{2}$O$_{5}$F', color='#161616', fontsize=18)
...
fig_one.savefig('exp_3d_87Rb_dist_RbTa2O5F.svg', format='svg', dpi=300, bbox_inches='tight', pad_inches=0.7)
plt.show()
