...
def generate_smooth_surface(input_file, elevation=25, azimuth=-125):
    ...
    # Set axis labels
#    ax.set_title('$^{87}$Rb $-$ RbTa$_2$O$_5$F (without 6, 7 & 9)', fontname = 'Times New Roman', fontsize = 26, y = 1)
    ...
    ax.text(x=5, y=1, z=2, s=r'$^{87}$Rb $-$ RbTa$_{2}$O$_{5}$F', color='#161616', fontsize=18)

    # Show the plot
    fig.savefig('th_3d_87Rb_dist_RbTa2O5F.svg', format='svg', dpi=300, bbox_inches='tight', pad_inches=0.7)
    plt.show()
...
# Example usage
input_file = 'figure_7-f_data.txt'  # replace with your actual input file path
generate_smooth_surface(input_file, elevation=90, azimuth=-125)
