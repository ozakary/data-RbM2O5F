...
def plot_experimental_spectrum(file_path, operating_frequency_mhz):
    ...
    ax.plot(
        experimental_data['Delta_ISO (ppm)'], 
        experimental_data['Intensity_Cont-1'] - 60000, 
        label='Line 1', 
        color='#2A9D8F'
    )
    ...
    ax.plot(
        experimental_data['Delta_ISO (ppm)'], 
        (experimental_data['Intensity_Exp'] - experimental_data['Intensity_Fit']) - 120000, 
    ...
    ax_inset_1 = inset_axes(ax, width="40%", height="30%", loc="upper left", borderpad=1)
    ...
    ax_inset_1.set_xlabel(r'$^{93}Nb$ $frequency$ / $ppm$')
    ax_inset_1.set_xlim(-1400, -900)
...
if __name__ == "__main__":
    experimental_file = 'figure_2-c_data.asc'
