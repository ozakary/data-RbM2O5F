...
def P(C_Q, eta_Q):
    return ((C_Q**4 * eta_Q) / (np.sqrt(2 * np.pi) * (27.53369**5))) * ( 1 - 
        (eta_Q**2 / 9)) * np.exp(- (C_Q**2 / (2*(27.53369**2))) * (1 + (eta_Q**2 / 3))) # 995.42 (RbTa) and 825.98 (RbNb) 87Rb and 25783.45 (RbNb) 93Nb

# Variables:
C_Q = np.linspace(0, 150, 50) #(0, 3000, 50) and (0, 100000, 50) for 87Rb and 93Nb respectively
...
canvas.set_zlabel('$P$ ×10$^{-2}$ / a.u.', fontsize = 20, labelpad=20)
...
canvas.set_zlim(0, 0.015)

# Adding text in the 3D plot
# Here, the text is added at the position (x, y, z) -> (3, 0.5, 0.3)
canvas.text(x=70, y=0.5, z=0.012, s=r'$^{93}$Nb $-$ RbNb$_{2}$O$_{5}$F', color='#161616', fontsize=18)
...
fig_one.savefig('exp_3d_93Nb_dist_RbNb2O5F.svg', format='svg', dpi=300, bbox_inches='tight', pad_inches=0.7)
plt.show()