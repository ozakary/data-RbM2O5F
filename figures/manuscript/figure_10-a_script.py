...
def plot_combined_and_experimental_spectrum(chunk_files, experimental_file, operating_frequency_mhz):
    ...
    ax.plot(combined_delta_iso, combined_intensity*0.365e4, label='Theoretical', color='#2A9D8F', linestyle='dashed')
    ...
    ax_inset_1.plot(
        experimental_data['Delta_ISO (ppm)'], 
        experimental_data['Intensity_scaled'] + 0.5e4, 
        color='#264653'
    )
    ax_inset_1.plot(combined_delta_iso, combined_intensity*0.365e4 + 0.5e4, color='#2A9D8F', linestyle='dashed')

    # Zoomed region settings
    ax_inset_1.set_xlabel(r'$^{87}Rb$ $frequency$ / $ppm$')
    ax_inset_1.set_xlim(-120, -40)
    ax_inset_1.set_ylim(0, 2.8e4)  # Adjust the y-limits if needed
    ...
    ax_inset_2.plot(
        experimental_data['Delta_ISO (ppm)'], 
        experimental_data['Intensity_Exp'] + 250, 
        linewidth=2.5,
        color='#264653'
    )
    ax_inset_2.plot(combined_delta_iso, combined_intensity*0.365e4 + 250, color='#2A9D8F', linestyle='dashed')
...
if __name__ == "__main__":
    experimental_file = 'figure_10-a_data-1.asc'
