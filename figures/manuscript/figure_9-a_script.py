...
def plot_combined_and_experimental_spectrum(chunk_files, experimental_file, operating_frequency_mhz):
    ...
    experimental_data['Intensity_scaled'] = experimental_data['Intensity_Exp'] - 200  # Scale if necessary
    ...
    ax.plot(
        ...
        experimental_data['Intensity_scaled'], 
        label='Experimental', 
        color='#264653'
    )

    # Plot the theoretical spectrum
    ax.plot(combined_delta_iso, combined_intensity*2120, label='Theoretical', color='#2A9D8F', linestyle='dashed')
    ...
    #Add asterisks
    ax.text(
    -42.5, 1700,  # x, y coordinates for the text
    '*',                           # Text to display
    ...
    ax.text(
    -158, 1700,  # x, y coordinates for the text
    '*',                           # Text to display
    fontsize=22,                               # Font size of the text
    color='#264653',                             # Text color
    ha='center',                               # Horizontal alignment (center, left, right)
    ...
    ax.text(
    13, 250,  # x, y coordinates for the text
    '*',                           # Text to display
    fontsize=22,                               # Font size of the text
    color='#264653',                             # Text color
    ha='center',                               # Horizontal alignment (center, left, right)
    ...
    ax.text(
    -213.5, 250,  # x, y coordinates for the text
    '*',                           # Text to display
    fontsize=22,                               # Font size of the text
    color='#264653',                             # Text color
    ha='center',                               # Horizontal alignment (center, left, right)
...
#    ax_inset = fig.add_axes(inset_position)
    ...
    ax_inset.plot(
        experimental_data['Delta_ISO (ppm)'], 
        experimental_data['Intensity_scaled'] + 8000, 
        color='#264653'
    )
    ax_inset.plot(combined_delta_iso, combined_intensity*2120 + 8000, color='#2A9D8F', linestyle='dashed')

    # Zoomed region settings
    ax_inset.set_xlabel(r'$^{19}F$ $frequency$ / $ppm$')
    ax_inset.set_xlim(-140, -60)
    ax_inset.set_ylim(0, max(combined_intensity*3000))  # Adjust the y-limits if needed
...
#    ax_inset.set_yticks([])  # Remove y-axis ticks in the inset
    ...
    plt.savefig("19F_th_vs_exp_RbNb2O5F.svg")  # Save the plot as a PDF
    plt.show()

# Main execution
if __name__ == "__main__":
    experimental_file = 'figure_9-a_data-1.asc'
