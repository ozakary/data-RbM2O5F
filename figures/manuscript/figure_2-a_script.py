...
def plot_experimental_spectrum(file_path, operating_frequency_mhz):
    ...
    ax.plot(
        experimental_data['Delta_ISO (ppm)'], 
        experimental_data['Intensity_Cont-1'] - 4000, 
        label=r'STSS ($n = 0$)', 
        color='#E76F51'
    )
    ...
    ax.plot(
        experimental_data['Delta_ISO (ppm)'], 
        (experimental_data['Intensity_Exp'] - experimental_data['Intensity_Fit']) - 6000, 
...
#    ax_inset.set_ylim(0, max(combined_intensity*50))  # Adjust the y-limits if needed
    ...
    ax_inset_2.set_ylim(0, 1000)  # Adjust the y-limits if needed
...
if __name__ == "__main__":
    experimental_file = 'figure_2-a_data.asc'
