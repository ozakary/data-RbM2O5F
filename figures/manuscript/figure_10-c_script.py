...
def plot_combined_and_experimental_spectrum(chunk_files, experimental_file, operating_frequency_mhz):
    ...
    experimental_data = pd.read_csv(
        ...
        names=['Frequency (Hz)', 'Intensity_Exp', 'Intensity_Fit', 'Intensity_Cont-1']  # Assign column names
    )
    ...
    ax.plot(combined_delta_iso, combined_intensity*4.65e5, label='Theoretical', color='#2A9D8F', linestyle='dashed')

    # Plot settings
    ax.set_xlabel(r'$^{93}Nb$ $frequency$ / $ppm$')
...
#    ax_inset = fig.add_axes(inset_position)
    ...
    ax_inset.plot(combined_delta_iso, combined_intensity*4.65e5, color='#2A9D8F', linestyle='dashed')

    # Zoomed region settings
    ax_inset.set_xlabel(r'$^{93}Nb$ $frequency$ / $ppm$')
    ax_inset.set_xlim(-1400, -900)
...
if __name__ == "__main__":
    experimental_file = 'figure_10-c_data-1.asc'
    operating_frequency_mhz = 208.118  # Operating frequency in MHz
