...
# Load the data
data_1 = pd.read_csv("./figure_6-a_data-1.csv")
data_2 = pd.read_csv("./figure_6-a_data-2.csv")
data_3 = np.loadtxt("./figure_6-a_data-3.txt", skiprows=1, usecols=range(1, 7 + 1))

# Sum the data from column 3 to the end
column_sum_1 = np.sum(data_1.iloc[:, 2:], axis=0)
column_sum_2 = np.sum(data_2.iloc[:, 2:])
...
bar_plot_3 = ax.bar(index + bar_width, Proba_conf(6) / 100, bar_width, color='#E76F51', alpha=1, label='Random')
...
plt.tight_layout()
fig.savefig('plot_configs_NbX6.svg', format='svg', dpi=300, bbox_inches='tight')
plt.show()

print(Proba_conf(6) / 100)
print(np.array(Prob_th_1)/100)
print(np.array(Prob_th_2)/100)
print(np.array(Prob_th_3)/100)
