...
def plot_experimental_spectrum(file_path, operating_frequency_mhz):
    ...
    ax.plot(
        experimental_data['Delta_ISO (ppm)'], 
        experimental_data['Intensity_Cont-1'] -200, 
        label='Line 1', 
        color='#2A9D8F'
    )
    ...
    ax.plot(
        experimental_data['Delta_ISO (ppm)'], 
        (experimental_data['Intensity_Exp'] - experimental_data['Intensity_Fit']) - 400, 
...
#    ax_inset.set_ylim(0, max(combined_intensity*50))  # Adjust the y-limits if needed
    ...
    ax_inset_2.set_xlim(-800, 0)
    ax_inset_2.set_ylim(-20, 250)  # Adjust the y-limits if needed
    ax_inset_2.invert_xaxis()  # Invert x-axis in the inset as well


    plt.tight_layout()
    plt.savefig("SI_figure_87Rb_RbF_exp_vs_fit_spectrum.pdf")  # Save the plot as a PDF
...
if __name__ == "__main__":
    experimental_file = 'figure_S2_data.asc'
    operating_frequency_mhz = 98.2174  # Operating frequency in MHz
