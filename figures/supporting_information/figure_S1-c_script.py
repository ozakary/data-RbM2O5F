...
data_1 = np.loadtxt("figure_S1-c_data.txt", skiprows = 1, usecols=(1, 2))
...
ax.text(x = -190, y = -150, s = text[0] + "$= {m:.3f}$".format(m=m11) + '$(16)$ ' + text[1] + ' $+$ ' + "${b:.1f}$".format(b=b11) + '$(1.8)$', color = '#264653', fontsize = 14.5)
ax.text(x = -30, y = -190, s = f'$R^2 = {results11.rsquared:.4f}$', color = '#264653', fontsize = 14.5)


# Show plot
plt.tight_layout()
fig.savefig('19F_iso_slop_RbTa2O5F.pdf', format='pdf', dpi=300, bbox_inches='tight')
plt.show()
